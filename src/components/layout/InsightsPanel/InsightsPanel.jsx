    ...
    return (
        <aside className="space-y-4 xl:sticky xl:top-22 xl:self-start" aria-label="Productivity insights">
            <Card className="p-5">
                ...
                <div className="mx-auto mt-4 grid size-32 place-items-center rounded-full" style={{ background: `conic-gradient(var(--app-primary) ${percentage}%, var(--app-control-muted) 0)` }}>
                    <div className="grid size-24 place-items-center rounded-full bg-card text-center"><div><strong className="font-display text-2xl text-foreground">{percentage}%</strong><span className="block text-[10px] text-muted">complete</span></div></div>
                </div>
                <p className="mt-4 text-center text-xs text-muted">{completedToday} of {todaysTasks.length} tasks completed today.</p>
            </Card>

            <Card className="p-5">
                <div className="flex items-center justify-between"><h3 className="font-display text-sm font-bold text-foreground">{new Intl.DateTimeFormat('en', { month: 'long', year: 'numeric' }).format(calendarDate)}</h3><div className="flex"><IconButton size="sm" aria-label="Previous month" onClick={() => changeMonth(-1)}><ChevronLeft className="size-3.5" /></IconButton><IconButton size="sm" aria-label="Next month" onClick={() => changeMonth(1)}><ChevronRight className="size-3.5" /></IconButton></div></div>
                ...
                <div className="mt-2 grid grid-cols-7 gap-1 text-center text-[10px]">
                    {calendarCells.map((day, index) => {
                        ...
                        return <span key={value} className={`relative grid aspect-square place-items-center rounded-md ${value === today ? 'bg-primary font-bold text-white' : 'text-body'}`}>{day}{hasTask && value !== today && <i className="absolute bottom-0.5 size-1 rounded-full bg-primary" />}</span>
                    })}
                </div>
            </Card>
